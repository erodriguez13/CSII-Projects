...
private:
    ...
    int gcd();
    // A private function that is used to find the gcd of numerator
...
Fraction::Fraction(int a, int b): num (a), den(b){
    if (b< 0) {num = a *-1; den=b*-1; }// moves the negative sign to numerator if the negative sign was at the bottom
}

Fraction::Fraction (int a): num (a), den(1){}// makes a fractin: a/1

Fraction::Fraction() : num(0), den(1){}// makes a default fractin: 0/1

int Fraction::get_numerator() const {
    return this-> num;
}// function that returns top number of current Fraction

int Fraction::get_denominator() const {
    return this-> den;
}// function that returns bottom number of current Fraction

void Fraction:: reduce(){// reduces fraction by diving top and bottom by gcd
    int g= gcd();
    num= num/g;
    den= den/g;
    if(den<0){
...
Fraction Fraction:: reciprocal() const {return Fraction (den, num);} // calls 2 parameter constructor switching the paramenter

Fraction operator +(const Fraction& f1, const Fraction& f2){// return the sum of f1 and f2, the result is reduced
    int a, b;
    a= (f1.get_numerator()*f2.get_denominator())+ (f2.get_numerator()*f1.get_denominator());
    b= f1.den * f2.den;
    Fraction temp (a, b);
    //temp.reduce();
    return temp;
...
Fraction operator -(const Fraction& f1, const Fraction& f2){// return the difference of f1 and f2, the result is reduced
    Fraction temp;
...
Fraction operator *(const Fraction& f1, const Fraction& f2){//return the product of f1 and f2, the result is reduced
    int a, b;
    a = f1.num * f2.num;
    b = f1.den * f2.den;
    Fraction temp (a, b);
...
Fraction operator /(const Fraction& f1, const Fraction& f2){// return the quotient of f1 and f2, the result is reduced
    Fraction temp;
    temp= f1* f2.reciprocal();
    temp.reduce();
    return temp;
}

Fraction operator -(const Fraction& f){// returns negative value of fraction
    int top= -1*f.get_numerator();
    int bottom= f.get_denominator();
...
// False otherwise
    int a, b;
    a= f1.num*f2.den;// comparisions need to be made under the same denominator
    b=f2.num*f1.den;
    if (a<b)return true;
    else return false;
